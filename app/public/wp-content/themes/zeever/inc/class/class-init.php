...
class Init {
	...
	private function load_hooks() {
		...
		add_filter( 'gutenverse_themes_template', array( $this, 'add_template' ), 10, 3 );
	 ...
	 * @param boolean $append_gutenverse_folder Append Gutenverse Folder.
	 *
	 * @return array
	 */
	public function add_template( $template_files, $template_type, $append_gutenverse_folder = true ) {
		$directory         = get_template_directory();
		$gutenverse_folder = $append_gutenverse_folder ? '/gutenverse-files' : '';
		...
		if ( 'wp_template' === $template_type ) {
			...
			foreach ( $new_templates as $template ) {
				$template_files[] = array(
					'slug'  => $template,
					'path'  => $directory . $gutenverse_folder . "/templates/{$template}.html",
					'theme' => get_template(),
					'type'  => 'wp_template',
				);
			}
